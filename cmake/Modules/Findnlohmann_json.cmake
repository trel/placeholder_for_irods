...
FIXME: Include directory chicanery
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

The current nlohmann-json extern package has ``json.hpp`` in its include
folder itself instead of in a nlohmann subdirectory of its include
folder. Consequently, the include directives throughout irods expect to
find ``jsoh.hpp`` in an include directory passed to the compiler, rather
than in a subdirectory. This is incompatible with the way nlohmann-json
is typically packaged. This module works around the issue, but this
...
macro(_nlohmann_json_fix_includes)
  ...
  find_path(
    ...
    NAMES "json.hpp"
    PATHS "${nlohmann_json_INCLUDE_DIRECTORIES}"
    PATH_SUFFIXES "nlohmann"
    NO_DEFAULT_PATH
  )
  if (NOT nlohmann_json_include_dir STREQUAL "nlohmann_json_include_dir-NOTFOUND" AND NOT nlohmann_json_include_dir IN_LIST nlohmann_json_INCLUDE_DIRECTORIES)
    list(APPEND nlohmann_json_INCLUDE_DIRECTORIES "${nlohmann_json_include_dir}")
  endif()
  unset(nlohmann_json_include_dir CACHE)
...
function(_nlohmann_json_create_target include_dir_var_name)
  if (NOT "${${include_dir_var_name}}" STREQUAL "${include_dir_var_name}-NOTFOUND")
    ...
    get_filename_component(include_dir_name "${${include_dir_var_name}}" NAME)
    if (include_dir_name STREQUAL "nlohmann")
      get_filename_component(include_dir_par "${${include_dir_var_name}}" DIRECTORY)
      set(nlohmann_json_INCLUDE_DIRECTORIES "${include_dir_par};${nlohmann_json_INCLUDE_DIRECTORIES}")
    endif()
    ...
    set(CMAKE_EXTRA_INCLUDE_FILES "json.hpp")

    # clear cached results if hash of json.hpp has changed
    file(MD5 "${${include_dir_var_name}}/json.hpp" nlohmann_json_HPP_MD5)
    ...
    set(nlohmann_json_CONFIG "non-package:${${include_dir_var_name}}/json.hpp")
    set(nlohmann_json_CONSIDERED_CONFIGS "${nlohmann_json_CONSIDERED_CONFIGS};non-package:${${include_dir_var_name}}/json.hpp" PARENT_SCOPE)
...
if (DEFINED IRODS_EXTERNALS_FULLPATH_JSON AND NOT TARGET nlohmann_json::nlohmann_json)
  ...
  else() # The irods-externals package probably won't have the cmake configuration files
    find_path(
      nlohmann_json_include_dir
      NAMES "json.hpp"
